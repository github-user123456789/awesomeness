...
function move(time)
	...
	if Humanoid.FloorMaterial == Enum.Material.Air and Humanoid:GetState() ~= Enum.HumanoidStateType.Freefall  and Humanoid:GetState() ~= Enum.HumanoidStateType.Climbing then -- custom
		if (Torso.Velocity * Vector3.new(1, 0, 1)).magnitude > 0 then
			pose = "Running"
		else
			pose = "Standing"
		end
